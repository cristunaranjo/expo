...
export async function resolveBuildModeAsync(
  ...
  let mode: EnvMode = variant.toLowerCase().endsWith('release') ? 'production' : 'development';
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const result = await queryBuildModeAsync(projectRoot, variant, systemEnv);
    if (result.mode === mode) {
      return result;
    }
    mode = result.mode;
  ...
  throw new CommandError(
    'ANDROID_BUILD_MODE',
    `Android variant '${variant}' changes mode between development and production environments. Configure react.debuggableVariants independently of mode-specific dotenv values.`
  );
}
