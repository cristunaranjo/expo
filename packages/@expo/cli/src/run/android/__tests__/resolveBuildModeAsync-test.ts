...
describe(resolveBuildModeAsync, () => {
  ...
  it('reloads the reported mode without retaining development values or changing the parent', async () => {
    vol.fromJSON(
      {
    ...
    const [first, second] = jest.mocked(spawnAsync).mock.calls;
    ...
    expect(second![2]!.env).toMatchObject({
      NODE_ENV: 'production',
      BABEL_ENV: 'production',
      EXPO_PUBLIC_MODE: 'production',
    });
    expect(second![2]!.env).not.toHaveProperty('ONLY_DEVELOPMENT');
    expect(process.env).toBe(parentEnv);
    expect(process.env).toEqual(parentValues);
  });

  it('keeps an explicit shell BABEL_ENV when the reported mode changes', async () => {
    process.env.BABEL_ENV = 'custom';
    mockReport({
      version: 1,
      projectPath: ':app',
      variant: 'qa',
    ...
    await resolveBuildModeAsync('/project', 'qa');

    expect(spawnAsync).toHaveBeenCalledTimes(2);
    for (const [, , options] of jest.mocked(spawnAsync).mock.calls) {
      expect(options!.env!.BABEL_ENV).toBe('custom');
    }
    expect(process.env.BABEL_ENV).toBe('custom');
  });

  it('resolves native debugging from common dotenv under the final production environment', async () => {
    ...
    expect(spawnAsync).toHaveBeenCalledTimes(2);
    expect(
      jest.mocked(spawnAsync).mock.calls.map(([, , options]) => options!.env!.NODE_ENV)
    ).toEqual(['development', 'production']);
    for (const [, , options] of jest.mocked(spawnAsync).mock.calls) {
      expect(options!.env!.EX_UPDATES_NATIVE_DEBUG).toBe('1');
    }
    expect(process.env).not.toHaveProperty('EX_UPDATES_NATIVE_DEBUG');
  });
  ...
  it('rejects a mode that changes again after loading its dotenv files', async () => {
    vol.fromJSON({ '.env.development': 'EX_UPDATES_NATIVE_DEBUG=1' }, '/project');
    mockNativeDebugReport();
    const parentEnv = process.env;
    const parentValues = { ...parentEnv };

    await expect(resolveBuildModeAsync('/project', 'debug')).rejects.toMatchObject({
      code: 'ANDROID_BUILD_MODE',
    });

    expect(spawnAsync).toHaveBeenCalledTimes(2);
    expect(jest.mocked(spawnAsync).mock.calls[1]![2]!.env).not.toHaveProperty(
      'EX_UPDATES_NATIVE_DEBUG'
    );
    expect(process.env).toBe(parentEnv);
    expect(process.env).toEqual(parentValues);
  ...
  it('preserves a Gradle failure after changing modes without retrying', async () => {
    const error = new Error('Gradle configuration failed');
    jest
      .mocked(spawnAsync)
    ...
    await expect(resolveBuildModeAsync('/project', 'qa')).rejects.toBe(error);
    expect(spawnAsync).toHaveBeenCalledTimes(2);
  });
